...
import { useBackgrounds, useBackgroundDetail } from '@/features/catalog/queries.js'

function BackgroundDetail({ backgroundId }) {
  const { data: detail, isLoading } = useBackgroundDetail(backgroundId || null)
  if (isLoading) {
    return (
  ...
  return (
    <div className="mt-2 space-y-4 border-t border-stone-800 px-[15px] pt-2 pb-2">
      ...
      {(detail.starting_items ?? []).length > 0 && (
        <div>
          <p className="mb-1.5 text-[11px] font-medium uppercase tracking-wide text-stone-500">Снаряжение</p>
          <ul className="list-disc space-y-0.5 pl-4 text-[11px] text-stone-400">
...
export default function BackgroundPickerModal({ currentId, onPick, onClose }) {
  const backgroundsQ = useBackgrounds()
  const backgrounds = backgroundsQ.data ?? []
  const items = [{ id: '', name: 'Без предыстории' }, ...backgrounds]
  ...
  return (
    <Modal title="Выбор предыстории" onClose={onClose} size="lg" scroll>
      <p className="mb-3 text-sm text-stone-400">
        Предыстория определяет, кем персонаж был до приключений, и даёт владение навыками и снаряжение.
      </p>
        ...
        {items.map((it) => {
          ...
          return (
            ...
            >
              ...
              {isOpen && it.id !== '' && <BackgroundDetail backgroundId={it.id} />}
            </div>
          )
        })}
      </div>
