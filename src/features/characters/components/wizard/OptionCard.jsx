export function OptionCard({ selected, disabled, onClick, title, subtitle, children, className = '' }) {
  return (
    ...
    >
      <span
        className={`absolute right-2.5 top-2.5 flex size-4 items-center justify-center rounded-full border text-[10px] ${
      ...
      <span className="pr-6">
        {title && <span className="block font-display text-sm font-semibold text-stone-100">{title}</span>}
